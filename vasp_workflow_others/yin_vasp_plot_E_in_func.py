...
def post_E_in(stype='slab'):
    ...
    natoms, a, t, tv = calc_a_t(latoms, jobn) 
    ...
    if stype == 'slab':
        check_slab_latt3(latoms)
        sys_name = '%s, $t_v \\geqslant %.2f~\\mathrm{\AA}$' %(sys_name, tv.min() ) 
   
    elif stype == 'bulk':
        check_bulk_latt3(latoms, t/(nlayers-1)*nlayers ) 
        sys_name = '%s, bulk'  %(sys_name) 



    plot_E_in(natoms, sys_name, Etot, V0, a, a0, t, t0)
...
def calc_A0(latoms, e):
    A = np.array([])
    for i in np.arange(len(latoms)):
        latt = latoms[i].get_cell()[:] / (e[i]+1)
...
def check_bulk_latt3(latoms, t2):
...
def plot_E_in(natoms, sys_name, Etot, V0, a, a0, t, t0):
    ...
    ylim = ax1[1].get_ylim()
    ax1[1].plot( np.array([0, 0]), ylim, '--k', alpha=0.2)
    ax1[1].set_ylim(ylim) 
    ...
    ax1[1].text( xlim[0]+(xlim[1]-xlim[0])*0.55, ylim[0]+(ylim[1]-ylim[0])*0.7, \
...
def write_E_in(natoms, nlayers, sys_name, jobn, Etot, a, t, tv, a0, t0, A0, V0):
    ...
    f.write('# Biaxial straining of transversely isotropic materials/slabs: \n' )
